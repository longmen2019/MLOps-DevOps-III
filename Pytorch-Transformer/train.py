...
#         }, model_filename)
def train_model(config):
    # ✅ Device selection: CUDA (NVIDIA), MPS (Apple), or CPU
    ...
    if device.type == 'cuda':
        ...
        print(f"Device name: {torch.cuda.get_device_name(index)}")
        print(f"Device memory: {torch.cuda.get_device_properties(index).total_memory / 1024 ** 3:.2f} GB")
